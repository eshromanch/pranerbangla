  ...
  if (items.name_en ==="About Us") {
    return (
      <Link
        key={items.id}
        href={`/about`}
        className="link link-hover text-base font-bold text-black dark:text-white dark:font-normal">আমাদের পরিচিতি</Link>
    );
  }
})}
{data?.map(items=>{
...
<Link
 key={items.id}
  href={`/Subscription`}
  className="link link-hover text-base font-bold text-black dark:text-white dark:font-normal">সাবস্ক্রিপসন</Link>
    );
  }
})}
...
<Link
 key={items.id}
  href={`/Contact`}
  className="link link-hover text-base font-bold text-black dark:text-white dark:font-normal">যোগাযোগ</Link>
    );
  }
})}
...
{data?.filter(e=> e.name_en !== "About Us" &&  e.name_en !== "Subscription" &&  e.name_en !== "Contact").map(items=>{
  return (
    <Link
      ...
      legacyBehavior><a className="link link-hover  text-base font-bold text-black dark:text-white dark:font-normal">{items.name_bn}</a></Link>
  );
})}
...
<p className='bold-case text-gray-800 text-base text-center md:text-sm dark:text-white '>স্বত্ব © ২০১৬ - ২০২২ প্রাণের বাংলা। সম্পাদক ও প্রকাশক: আবিদা নাসরীন কলি।</p>
  <p className='bold-case text-gray-800 font-enbody text-center text-base md:text-xs dark:text-white'><b>Email:</b> <b>Article:</b> <a href="mailto:amar@pranerbangla.com">amar@pranerbangla.com</a>, <b>Avertising:</b> <a href="mailto:ad@pranerbangla.com">ad@pranerbangla.com</a>
</p>
<p className='bold-case font-enbody  text-gray-800 text-base md:text-xs dark:text-white'><b>Phone:</b> <a href="tel:8801818189677">+8801818189677</a>, <a href="tel:8801717256199">+8801717256199</a></p>
</div>
      </div>
    );
}
