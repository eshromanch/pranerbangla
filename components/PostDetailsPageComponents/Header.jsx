...
const date =new Date(`${props.time}`).toLocaleString('en-uk',{day:"numeric",month:'short', year:'numeric'})
  return <div className='grid grid-cols-2 w-full gap-8 py-8 border-b-2 md:flex md:flex-col'>
                <div className='flex flex-col h-full w-full'>
                {props.postCat.map(item=>{
                  
 return <Link key={item.id} href={"/categories/"+item.category_id}><a><button className='btn btn-primary rounded-none text-lg font-normal w-1/4 m-4 transition ease-in-out   hover:bg-[#4b5767] hover:border-0 hover:rounded-lg duration-400'>{item.category.name_bn}</button></a></Link>
})
}
                <h1 className='text-5xl font-bold text-black dark:text-white  m-4 '>{props.title}</h1>

    <div className='flex my-8 '>
   <Link href={`/profile/${props.writerId}`}><a href="">
   <div className="relative  w-28 h-full  ">
    ...
    <Link href={`/profile/${props.writerId}`}><a href=""><h1 className='font-bold text-2xl text-black dark:text-white'>{props.user.name}</h1></a></Link>
