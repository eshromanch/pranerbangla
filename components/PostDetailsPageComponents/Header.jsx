...
const date =new Date(`${props.time}`).toLocaleString('en-uk',{day:"numeric",month:'short', year:'numeric'})
 ...
 return <Link href={"/categories/"+item.category_id}><a><button className='btn btn-primary rounded-none text-lg font-normal w-1/4 m-4 transition ease-in-out   hover:bg-[#4b5767] hover:border-0 hover:rounded-lg duration-400'>{item.category.name_bn}</button></a></Link>
})
}
